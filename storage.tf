...
RESOURCE_GROUP_NAME="josh-azureservices-rg"
STORAGE_ACCOUNT_NAME="josh-storage-account"
...
if [ "$STORAGE_ACCOUNT_EXISTS" = "false" ]; then
  ...
  az storage container create --name tfstate --account-name joshpstorageaccount
fi